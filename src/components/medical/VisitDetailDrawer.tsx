...
const VisitDetailDrawer = ({
  ...
  const [prescEditing, setPrescEditing] = useState<VisitPrescription | null>(
    null,
  );
  ...
  const handlePrescItemAdd = async (data: {
            ...
            {subAtts.map((a) => (
              <div key={a.id} className="flex items-center gap-2 text-xs">
                ...
                <Button
                  variant="ghost"
                  size="icon-xs"
                  onClick={() => handleVisitAttachmentDelete(a.id)}
                  title={t("common.delete")}
                >
                  <TrashIcon className="size-3.5" />
                </Button>
            ...
            {subInvs.map((inv) => (
              <div key={inv.id} className="flex items-center gap-2 text-xs">
                ...
                <Button
                  ...
                  size="icon-xs"
                  onClick={() => {
                    setViewingInvoiceId(inv.invoiceId);
                ...
                <Button
                  variant="ghost"
                  size="icon-xs"
                  onClick={() => handleInvoiceUnbind(inv.id)}
