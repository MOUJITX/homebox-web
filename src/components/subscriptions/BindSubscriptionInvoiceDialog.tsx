...
const BindSubscriptionInvoiceDialog = ({ recordId, boundInvoiceIds, open, onClose, onSuccess }: BindSubscriptionInvoiceDialogProps) => {
  ...
  useEffect(() => { setPage(0); }, [debouncedSearch, filterType, filterStatus]);
      ...
      <DialogContent className="sm:max-w-lg max-h-[80vh] flex flex-col">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            <Input placeholder={t("subscriptions.searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
              ...
              <Select value={filterType} onValueChange={(v) => setFilterType(v)}>
                <SelectTrigger>
                  <SelectValue placeholder={t("invoices.filters.allTypes")}>
                    {() => filterType == null ? t("invoices.filters.allTypes") : t(`invoices.type.${filterType}`)}
                  </SelectValue>
                </SelectTrigger>
                <SelectPopup>
                  <SelectItem value={null}>{t("invoices.filters.allTypes")}</SelectItem>
                  {INVOICE_TYPES.map((type) => (
                    <SelectItem key={type} value={type}>{t(`invoices.type.${type}`)}</SelectItem>
                  ))}
                </SelectPopup>
              </Select>
            </div>
              ...
              <Select value={filterStatus} onValueChange={(v) => setFilterStatus(v)}>
                <SelectTrigger>
                  <SelectValue placeholder={t("invoices.filters.allStatuses")}>
                    {() => filterStatus == null ? t("invoices.filters.allStatuses") : t(`invoices.status.${filterStatus}`)}
                  </SelectValue>
                </SelectTrigger>
                <SelectPopup>
                  <SelectItem value={null}>{t("invoices.filters.allStatuses")}</SelectItem>
              ...
              {availableInvoices.map((inv) => (
                <button
                  ...
                  className={`w-full text-left rounded-md border px-3 py-2 text-sm transition-colors ${
                    selectedId === inv.id ? "border-primary bg-primary/5" : "hover:bg-accent/50"
                  }`}
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-xs truncate">{inv.sellerName ?? t(`invoices.type.${inv.invoiceType}`)}</span>
                    </div>
                    {inv.totalAmount != null && <span className="text-xs font-medium shrink-0">{formatCurrency(inv.totalAmount)}</span>}
                  </div>
                </button>
              ))}
            </div>
