...
const SubscriptionDialog = ({
  ...
  const [subscriptionType, setSubscriptionType] =
    useState<SubscriptionType>("PERIODIC");
  ...
  const handleSubmit = async (e: SubmitEvent) => {
    ...
    try {
      const data: SubscriptionRequest = {
        ...
        status,
        renewNoticeDays:
          subscriptionType === "PERIODIC" ? renewNoticeDays : undefined,
        ...
        <div className="flex-1 overflow-y-auto p-1">
          ...
          >
            ...
            {subscriptionType === "PAY_AS_YOU_GO" && (
              <div className="grid gap-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={t("subscriptions.form.billingMode")}
                    >
                      {() =>
                        billingMode
                          ? t(
                              `subscriptions.billingModes.${billingMode === "PREPAID" ? "prepaid" : "postpaid"}`,
                            )
                          : t("subscriptions.form.billingMode")
                      }
                    </SelectValue>
                  </SelectTrigger>
                  <SelectPopup>
                    {BILLING_MODES.map((mode) => (
                      <SelectItem key={mode} value={mode}>
                        {t(
                          `subscriptions.billingModes.${mode === "PREPAID" ? "prepaid" : "postpaid"}`,
                        )}
                      </SelectItem>
                    ))}
                  </SelectPopup>
            ...
            {subscriptionType === "PERIODIC" && (
              <div className="grid gap-2">
                <Label htmlFor="renewNoticeDays">
                  {t("subscriptions.form.renewNoticeDays")}
                </Label>
                <Input
                  id="renewNoticeDays"
                  type="number"
                  ...
                  onChange={(e) => setRenewNoticeDays(Number(e.target.value))}
                />
              </div>
            )}
