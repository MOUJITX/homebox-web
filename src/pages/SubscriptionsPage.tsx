...
import type { Subscription, SubscriptionType, SubscriptionStatus, Page } from "@/api/subscriptions";
...
const statusBadgeVariant = (status: SubscriptionStatus): "success" | "secondary" | "outline" => {
  switch (status) {
...
const SubscriptionsPage = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<SubscriptionStatus | null>(null);
  ...
  useEffect(() => {
    ...
    if (subIdParam) {
      ...
      if (!Number.isNaN(id)) {
        ...
        navigate(query ? `/subscriptions?${query}` : "/subscriptions", { replace: true });
      }
    }
  }, []);
  ...
  const handleOpenDrawer = (subId: number) => { setDrawerSubId(subId); setDrawerOpen(true); };
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={filterType} onValueChange={(v) => { setFilterType(v); setPage(0); }}>
            <SelectTrigger>
              <SelectValue placeholder={t("subscriptions.filters.allTypes")}>
                {() => filterType == null ? t("subscriptions.filters.allTypes") : t(`subscriptions.types.${filterType === "PAY_AS_YOU_GO" ? "payAsYouGo" : "periodic"}`)}
              </SelectValue>
            </SelectTrigger>
            <SelectPopup>
              <SelectItem value={null}>{t("subscriptions.filters.allTypes")}</SelectItem>
          ...
          <Select value={filterStatus} onValueChange={(v) => { setFilterStatus(v); setPage(0); }}>
            <SelectTrigger>
              <SelectValue placeholder={t("subscriptions.filters.allStatuses")}>
                {() => filterStatus == null ? t("subscriptions.filters.allStatuses") : t(`subscriptions.status.${filterStatus.toLowerCase()}`)}
              </SelectValue>
            </SelectTrigger>
            <SelectPopup>
              <SelectItem value={null}>{t("subscriptions.filters.allStatuses")}</SelectItem>
              {STATUSES.map((s) => (
                <SelectItem key={s} value={s}>{t(`subscriptions.status.${s.toLowerCase()}`)}</SelectItem>
              ))}
            </SelectPopup>
          </Select>
        </div>
        ...
        <div className="w-40">
          <Select value={filterPlatformId} onValueChange={(v) => { setFilterPlatformId(v); setPage(0); }}>
            <SelectTrigger>
              <SelectValue placeholder={t("subscriptions.filters.allPlatforms")}>
                {() => filterPlatformId == null ? t("subscriptions.filters.allPlatforms") : (platforms.find((p) => p.id === filterPlatformId)?.name ?? t("subscriptions.filters.allPlatforms"))}
              </SelectValue>
            </SelectTrigger>
            <SelectPopup>
              <SelectItem value={null}>{t("subscriptions.filters.allPlatforms")}</SelectItem>
              {platforms.map((p) => (
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t("subscriptions.table.latestRecord")}</TableHead>
              <TableHead className="text-right">{t("subscriptions.table.endDate")}</TableHead>
          ...
          <TableBody>
            ...
            {!isLoading && data.content.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="h-24 text-center text-muted-foreground">{t("subscriptions.noSubscriptions")}</TableCell>
              </TableRow>
            )}
            {!isLoading && data.content.map((sub) => (
              <TableRow key={sub.id} className="cursor-pointer" onClick={() => handleOpenDrawer(sub.id)}>
                ...
                <TableCell>
                  <span className="text-xs">
                    {t(`subscriptions.types.${sub.subscriptionType === "PAY_AS_YOU_GO" ? "payAsYouGo" : "periodic"}`)}
                  </span>
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-right tabular-nums">
                  {sub.latestRecordAmount != null ? (
                    <span>
                      <span className="text-xs text-muted-foreground">{sub.latestRecordDate ? formatDate(sub.latestRecordDate) : ""}</span>
                      <br />
                ...
                <TableCell className="text-right tabular-nums text-xs">
                  {sub.latestRecordEndDate && sub.subscriptionType === "PERIODIC"
      ...
      <SubscriptionDialog open={createOpen} onClose={() => setCreateOpen(false)} />
      <SubscriptionDialog open={!!editingSubscription} subscription={editingSubscription} onClose={() => setEditingSubscription(null)} />
      <DeleteSubscriptionDialog open={!!deletingSubscription} subscription={deletingSubscription} onClose={() => setDeletingSubscription(null)} />
      <SubscriptionDetailDrawer
        subscriptionId={drawerSubId}
        open={drawerOpen}
        onClose={() => { setDrawerOpen(false); setDrawerSubId(null); }}
      />
